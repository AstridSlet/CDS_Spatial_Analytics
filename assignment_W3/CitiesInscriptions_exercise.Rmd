...
If nothing else is define the default measurement unit is in meters. 
...
tm_shape(ten_buffers)  +
  ...
  tm_shape(ten_cities) +
  tm_text("Ancient Toponym", size = 0.8, auto.placement = 5) +
  tm_dots(col = "blue", 
             size = 0.1) +
  ...
  tm_credits(position = c("RIGHT", "top"),
             text = "Astrid S. Rybner") +
  tm_layout(main.title = "10 ancient cities",
            bg.color = "#ADD8E6",
...
# use regeex to clean longitude and latitude column
i_sm <- i_sm %>% 
  mutate(longitude = as.numeric(str_extract(i_sm$longitude, pattern = "\\d+.\\d+")), 
         latitude = as.numeric(str_extract(i_sm$latitude, pattern = "\\d+.\\d+")))

# subset top 100 rows
i_sm <- slice(i_sm, n = 0:100)
...
# yep, looks like they are all in italy 
...
# seperate collummn in 2
i <- inscriptions %>% 
  separate(col = coordinates, into = c("longitude","latitude"), sep = ",")

# clean collumns
i <- i %>% 
  mutate(longitude = as.numeric(str_extract(i$longitude, pattern = "\\d+.\\d+")), 
         latitude = as.numeric(str_extract(i$latitude, pattern = "\\d+.\\d+")))
# check that collumn is indeed numeric
class(i$longitude)

# check number of rows in i      
nrow(i) # 12266

# filter out NA's
i <- i %>% filter(!is.na(longitude)) %>%
